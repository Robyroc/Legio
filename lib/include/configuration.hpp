...
#else
#define HANDLE_BCAST_FAIL(C)                             \
    printf("##### Broadcast failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_SEND_FAIL(C)                         \
    printf("##### Send failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_RECV_FAIL(C)                            \
    printf("##### Receive failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_REDUCE_FAIL(C)                         \
    printf("##### Reduce failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_GET_FAIL(C)                         \
    printf("##### Get failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_PUT_FAIL(C)                         \
    printf("##### Put failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_GATHER_FAIL(C)                         \
    printf("##### Gather failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define HANDLE_SCATTER_FAIL(C)                         \
    printf("##### Scatter failed, stopping a node\n"); \
    raise(SIGINT)
#endif
...
#else
#define PERFORM_GATHER(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, ROOT, COMM, \
                       TOTALSIZE, FAKERANK, FAKECOMM)                                          \
    ({                                                                                         \
        ...
        MPI_Win_fence(0, win);                                                                 \
        MPI_Put(SENDBUF, SENDCOUNT, SENDTYPE, ROOT, FAKERANK* RECVCOUNT, RECVCOUNT, RECVTYPE,  \
                win);                                                                          \
        MPI_Win_fence(0, win);                                                                 \
        MPI_Win_free(&win);                                                                    \
        rc = MPI_SUCCESS;                                                                      \
...
#else
#define PERFORM_SCATTER(SENDBUF, SENDCOUNT, SENDTYPE, RECVBUF, RECVCOUNT, RECVTYPE, ROOT, COMM, \
                        TOTALSIZE, FAKERANK, FAKECOMM)                                          \
    ({                                                                                          \
        int type_size, cur_rank;                                                                \
        MPI_Win win;                                                                            \
        ...
        MPI_Win_fence(0, win);                                                                  \
        MPI_Get(RECVBUF, RECVCOUNT, RECVTYPE, ROOT, FAKERANK* SENDCOUNT, SENDCOUNT, SENDTYPE,   \
                win);                                                                           \
        MPI_Win_fence(0, win);                                                                  \
        MPI_Win_free(&win);                                                                     \
        rc = MPI_SUCCESS;                                                                       \
