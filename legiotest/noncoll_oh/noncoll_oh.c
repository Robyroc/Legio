...
int main(int argc, char** argv)
{
    ...
    if(rank < group_size)
    {
        ...
        {
            recv_buf /= group_size;
            fprintf(file_p, "%d, %d, %s, %f, %f, %f\n", group_size, size, "mpi_group", recv_buf, recv_buf2, recv_buf3);
        }
    }
    for(int i = 0; i < MULT; i++)
        if(rank < group_size)
    ...
    if(rank < group_size)
    {
        ...
        {
            recv_buf /= group_size;
            fprintf(file_p, "%d, %d, %s, %f, %f, %f\n", group_size, size, "pmpi_group", recv_buf, recv_buf2, recv_buf3);
        }
    }
    for(int i = 0; i < MULT; i++)
        if(rank < group_size)
    ...
    if(rank < group_size)
    {
        ...
        {
            recv_buf /= group_size;
            fprintf(file_p, "%d, %d, %s, %f, %f, %f\n", group_size, size, "mpi_comm", recv_buf, recv_buf2, recv_buf3);
        }
    }
    ...
    */
    ...
    if(rank < group_size)
    {
        ...
        {
            recv_buf /= group_size;
            fprintf(file_p, "%d, %d, %s, %f, %f, %f\n", group_size, size, "pmpi_comm", recv_buf, recv_buf2, recv_buf3);
        }
    }
