...
int main(int argc, char* argv[])
{
    ...
    rc = MPI_Session_init(MPI_INFO_NULL, MPI_ERRORS_RETURN, &lib_shandle);
    if (rc != MPI_SUCCESS)
    {
        printf("A\n");
        return -1;
     ...
     * get a communicator
     */
    rc = MPI_Comm_create_from_group(wgroup, "mpi.forum.example", MPI_INFO_NULL, MPI_ERRORS_RETURN,
                                    &lib_comm);
    if (rc != MPI_SUCCESS)
    {
    ...
    if (0 == myid)
    {
        switch (po_ret)
        {
            case PO_CUDA_NOT_AVAIL:
                fprintf(stderr,
                        "CUDA support not enabled.  Please recompile "
                        "benchmark with CUDA support.\n");
                break;
            case PO_OPENACC_NOT_AVAIL:
                fprintf(stderr,
                        "OPENACC support not enabled.  Please "
    ...
    for (size = options.min_message_size; size <= options.max_message_size; size *= 2)
    {
        ...
        if (myid == 0)
        {
            for (i = 0; i < options.iterations + options.skip; i++)
            {
                ...
                {
                    MPI_CHECK(MPI_Irecv(r_buf, size, MPI_CHAR, 1, 10, lib_comm, recv_request + j));
                }

                for (j = 0; j < window_size; j++)
                {
                    MPI_CHECK(MPI_Isend(s_buf, size, MPI_CHAR, 1, 100, lib_comm, send_request + j));
                }
        ...
        else if (myid == 1)
        {
            for (i = 0; i < options.iterations + options.skip; i++)
            {
                for (j = 0; j < window_size; j++)
                {
                    MPI_CHECK(MPI_Irecv(r_buf, size, MPI_CHAR, 0, 100, lib_comm, recv_request + j));
                }

                for (j = 0; j < window_size; j++)
                {
                    MPI_CHECK(MPI_Isend(s_buf, size, MPI_CHAR, 0, 10, lib_comm, send_request + j));
                }
